...
def build_case_summary_prompt(case_text: str, target_words: int, max_words: int) -> str:
    return f"""You are an experienced Family Lawyer in Australia, accredited by the State/Territory Law Society as a Specialist in Family Law.
You are reading an AustLII decision and producing a STRICT JSON summary for retrieval by other family law practitioners.
...
- Parenting (CONDUCT): Look for any conduct by either party that the judge commented on or relied upon — including but not limited to: withholding of children, unilateral decisions, contravention of orders, alienation, gatekeeping, cooperative or facilitative conduct, travel/passport disputes, Hague Convention references, and Airport Watch List orders. Capture HOW the court characterised the conduct, WHAT consequences followed, and any judicial guidance or warning.
...
- Notable conduct: If the judge commented on, relied upon, or criticised any party's behaviour, capture: (a) what the party did, (b) the judge's characterisation, (c) the legal consequence, (d) the statutory basis if cited, and (e) any judicial guidance or warning that a practitioner could cite.
...
{{{{
  ...
  "children_parenting": {{{{
    ...
    "notable_conduct_and_judicial_commentary": ["Any significant conduct by either party that the judge commented on, relied upon, or criticised in reaching the decision. For each instance, capture: (a) what the party did, (b) how the judge characterised or labelled the conduct, (c) the legal consequence or weight given to it, and (d) any judicial guidance, warning, or principle stated. Examples (illustrative, not exhaustive): 'Mother withheld child from weekend handover citing illness with no medical evidence — judge characterised as deliberate frustration of father's time, resulting in makeup time order'; 'Father failed to disclose $200k trust asset — judge found pattern of non-disclosure undermined credibility on all financial issues'; 'Mother actively facilitated phone contact and school involvement — judge noted this as strong positive factor under s 60CC(3)(c)'; 'Father unilaterally relocated children interstate during proceedings — judge characterised as attempt to create status quo, ordered return'; 'Wife's repeated adjournment applications — judge found tactical delay, awarded indemnity costs'. Capture any conduct the court treated as significant — parenting, financial, procedural, or otherwise."],
